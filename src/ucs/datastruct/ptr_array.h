...
/**
 * Gets the super structure of a ptr_array.
 *
 * @param [in] _locked_ptr_array   Pointer a locked ptr array.
 *
 * @return The super structure of the locked ptr_array (i.e. ptr_array).
 */
#define ucx_ptr_array_locked_super(_locked_ptr_array) \
    ((ucs_ptr_array_t *)&(_locked_ptr_array)->super)
...
{
    ucs_ptr_array_t *ptr_array = ucx_ptr_array_locked_super(locked_ptr_array);

    return ucs_ptr_array_get_size(ptr_array);
}
...
{
    int is_continue_loop =
        ((element_index) < (ucx_ptr_array_locked_super(locked_ptr_array)->size));
...
#define ucs_ptr_array_locked_for_each(_var, _index, _locked_ptr_array) \
    ...
    for ((_index) = 0; \
         __ucx_ptr_array_locked_foreach_finalize(_locked_ptr_array, (_index)); \
             (_index) += __ucs_ptr_array_for_each_get_step_size((ucx_ptr_array_locked_super(_locked_ptr_array)), (_index))) \
        if ((ucs_likely(!__ucs_ptr_array_is_free( \
            (ucs_ptr_array_elem_t)((_var) = \
            (void *)((ucx_ptr_array_locked_super(_locked_ptr_array))->start[(_index)]))))))
